...
if [ "$2" == "all" ]; then
  pods=`kubectl get pods $namespace | grep Running | awk '{print $1}'`
else
  pods=`kubectl get pods $namespace $pod_to_kill | grep -v NAME | awk '{print $1}'`
fi

for pod in $pods; do
  containers_in_pod=`kubectl get pod $namespace $pod | grep Running | awk '{print $2}' | cut -d "/" -f2`
  for i in $(seq 0 $(($containers_in_pod-1))); do
    container_id=`kubectl get pod $namespace $pod -o jsonpath="{.status.containerStatuses[$i].containerID}" | cut -d '/' -f3;`
    container_name=`kubectl get pod $namespace $pod -o jsonpath="{.status.containerStatuses[$i].name}"`
    node=`kubectl get pod $namespace $pod -o wide | awk '/node/ {print $7}'`
